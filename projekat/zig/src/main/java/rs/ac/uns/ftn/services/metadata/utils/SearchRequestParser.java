...
public class SearchRequestParser {
	...
	private static String parsePrilog(String operand) {
		String[] pair = operand.split("=");
		return "( " + 
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_DOKAZ_O_PRAVU_PRVENSTVA + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_DOKAZ_O_UPLATI_TAKSE + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_OPSTI_AKT_O_KOLEKTIVNOM_ZIGU_GARANCIJE + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_PRIMERAK_ZNAKA + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_PUNOMOCJE + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_PUNOMOCJE_NAKNADNO_DOSTAVLJENO + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_PUNOMOCJE_RANIJE_PRILOZENO + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + " || " +
			   "regex(UCASE(str(?" + MetadataKeys.PRILOG_SPISAK_ROBE_I_USLUGA + ")), UCASE(\".*"+ pair[1].trim()+".*\"))" + 
			   " )";
	}
